...
public class PhotoUtils {
    ...
    public static void cutPhoto(Activity activity, Uri srcUri, Uri saveUri, Boolean hasRatio, int outWidth, int outHeight, int requestCropCode) {
        Intent intent = getCropIntent(srcUri, saveUri, hasRatio, outWidth, outHeight);
     ...
     * @return
     */
    private static Intent getCropIntent(Uri srcUri, Uri saveUri, boolean hasRatio, int outWidth, int outHeight) {
        if (srcUri == null) {
...
//            String url = getPath(getContext(), inputUri);//这个方法是处理4.4以上图片返回的Uri对象不同的处理方法
